...
namespace autopilot
{
    ...
    void 信号順守::経過(const 共通状態 &状態)
    {
        ...
        _現在信号速度 = _現在閉塞.信号速度;
        if (_現在閉塞.信号速度 + static_cast<mps>(2.0_kmph) <= 状態.現在速度())ATCブレーキ動作 = true;
        else if (状態.現在速度() <= _現在閉塞.信号速度 - static_cast<mps>(1.5_kmph))ATCブレーキ動作 = false;
    }

    自動制御指令 信号順守::出力ノッチ(const 共通状態 &状態) const
    {
        if (is_atc() && !_現在閉塞.停止解放 && _現在閉塞.信号速度 == 0.0_mps)
        {
            自動制御指令 停止出力ノッチ = 自動制御指令{ 自動制動自然数ノッチ{0} };
            return 停止出力ノッチ;
        }
        自動制御指令 ノッチ = std::min(
            _信号グラフ.信号出力ノッチ(状態, ATCブレーキ動作), _照査グラフ.出力ノッチ(状態));
