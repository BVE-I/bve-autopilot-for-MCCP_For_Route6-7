...
- TASC
  ...
  - [x] TASCブレーキ緩解条件で手動ブレーキを1段でも入れれば緩む設定を追加
  - [x] ~~オン/オフ切り替え時に音を鳴らす~~
- ATO
  - [x] 速度の自動制御
    - [x] 独自地上子による制限速度の設定
  ...
  - [x] メトロ総合プラグイン互換モード
    - [x] 前方予告に基づく速度制御
    - [x] ORP に沿う速度制御
    - [x] ATCブレーキが動作している時に、ATOが過剰にブレーキをかける挙動を削除
    - [x] ATO出発時にブレーキの緩み方、ノッチの上げ方をiniファイルで設定できるように変更
    - [x] ATO出発が非常ブレーキでリセットされる条件を追加
    - [x] ATO運転中に停止現示で停止した際に常用最大ブレーキを出力できるように修正
...
**このforkはメトロ総合プラグイン準拠6・7号統合プラグイン Metro-Compliant-Comprehensive-Plugin(MCCP)-For-Route6-and-7の併用を前提に改造しています。このプラグインの前にメトロ総合プラグイン準拠6・7号統合プラグイン Metro-Compliant-Comprehensive-Plugin(MCCP)-For-Route6-and-7を読み込ませてください。**
